...
# Настройки ротации: максимум 500MB на файл, 3 файла бэкапа = до 1.5GB логов
max_bytes = 500 * 1024 * 1024  # 500 MB
backup_count = 3  # Храним 3 ротированных файла
...
class SuperBotV4MTF:
    ...
    async def smart_symbol_selection_v4(self, market_data: Dict) -> List[str]:
        ...
        try:
            ...
            # Вставляем приоритетные монеты в начало списка
            final_symbols = []
            seen = set()
    ...
    async def _fetch_ohlcv(self, symbol: str, timeframe: str, limit: int = 100) -> pd.DataFrame:
        ...
        for _ in range(attempts):
            try:
                ...
                last_err = e
        # Фолбэк для 45m: пробуем альтернативные источники
        if timeframe == '45m':
            # Фолбэк #1: Binance API (поддерживает 45m)
            try:
                import ccxt
                ...
                normalized_symbol_binance = normalized_symbol.replace('USDT', '/USDT:USDT') if normalized_symbol.endswith('USDT') else normalized_symbol
                ohlcv_binance = await binance.fetch_ohlcv(normalized_symbol_binance, '45m', limit=limit)
                if ohlcv_binance:
                    df = pd.DataFrame(ohlcv_binance, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
                    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
                    logger.debug(f"✅ 45m: Получено через Binance API для {symbol}")
                    return df
            except Exception as e:
                logger.debug(f"⚠️ Binance fallback для 45m не удался: {e}")
            
            # Фолбэк #2: OKX API (поддерживает 45m)
            try:
                import ccxt
                okx = ccxt.okx({
                    'enableRateLimit': True,
                ...
                normalized_symbol_okx = normalized_symbol.replace('USDT', '/USDT:USDT') if normalized_symbol.endswith('USDT') else normalized_symbol
                ohlcv_okx = await okx.fetch_ohlcv(normalized_symbol_okx, '45m', limit=limit)
                if ohlcv_okx:
                    df = pd.DataFrame(ohlcv_okx, columns=['timestamp', 'open', 'high', 'low', 'close', 'volume'])
                    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
                    logger.debug(f"✅ 45m: Получено через OKX API для {symbol}")
                    return df
            except Exception as e:
                logger.debug(f"⚠️ OKX fallback для 45m не удался: {e}")
            
            # Фолбэк #2: Синтезируем 45м из 15м (наиболее надежный метод)
            try:
                ...
                if not df15.empty:
                    ...
                    logger.debug(f"✅ 45m: Синтезировано из 15m для {symbol} ({len(agg)} свечей) - математически корректен")
    ...
    def _get_adaptive_signal_params(self, market_condition: str, symbol_data: Dict, 
        ...
        try:
            ...
            # УЖЕСТОЧЕНО: Повышаем минимальный порог уверенности до 70-85% (было 65-85)
            adaptive_min_confidence = max(70, min(85, adaptive_min_confidence))
    ...
    async def analyze_symbol_v4(self, symbol: str) -> Optional[EnhancedSignal]:
        ...
        try:
            ...
            # Для бокового рынка считаем тренд нейтральным
            if is_sideways:
                global_trend_bullish = True  # Разрешаем обе стороны
                global_trend_bearish = True   # Разрешаем обе стороны
            ...
            try:
                ...
                if not df_45m.empty and len(df_45m) > 20:
                    manipulation_result = ManipulationDetector.detect_manipulation(df_45m, current_45m)
                    if manipulation_result:
                        ...
                        # Манипуляции имеют приоритет, НО ВСЕ РАВНО требуют MTF подтверждения 45m+1h+4h
                        ...
                        # Временная проверка MTF для манипуляций
                        ...
                        if mtf_ok:
                            ...
                            # УЖЕСТОЧЕНО: Для манипуляций тоже требуем минимум 65% (было 55%)
                            adaptive_min_confidence = max(adaptive_params.get('min_confidence', 70) - 5, 65)
            ...
            # Проверяем Bollinger Reversion с 45m подтверждением (если манипуляций нет)
            if not signal and current_45m:
            ...
            # Если нет BB сигнала и манипуляций, проверяем обычные условия
            # УЖЕСТОЧЕНО: требуется минимум 8 из 10 условий (было 7)
            if not signal:
                buy_count = sum(buy_normal.values())
                sell_count = sum(sell_conditions.values())
                
                if buy_count >= 8:  # Минимум 8 из 10 условий (было 7)
                    signal = 'buy'
                    potential_direction = 'buy'
                    # Повышаем базовую уверенность: минимум 60% (было 50%)
                    confidence = 60 + (buy_count - 8) * 5
                    reasons = ['BUY-NORMAL_V4', f'Conditions:{buy_count}/10']
                elif sell_count >= 8:  # Минимум 8 из 10 условий (было 7)
                    signal = 'sell'
                    potential_direction = 'sell'
                    # Повышаем базовую уверенность: минимум 60% (было 50%)
                    confidence = 60 + (sell_count - 8) * 5
            ...
            # 🔒 Жёсткий фильтр: подтверждение 45m + 1h + 4h по направлению (УЖЕСТОЧЕНО)
            def _mtf_confirm(dir_: str) -> bool:
                if dir_ == 'buy':
                    # УЖЕСТОЧЕНО: Проверяем EMA + цена выше EMA21 на всех таймфреймах
                    price_45m = current_45m.get('price', 0)
                    price_1h = current_1h.get('price', 0)
                    price_4h = current_4h.get('price', 0)
                    ema21_45m = current_45m.get('ema_21', 0)
                    ema21_1h = current_1h.get('ema_21', 0)
                    ema21_4h = current_4h.get('ema_21', 0)
                    
                    c45 = current_45m.get('ema_9', 0) > ema21_45m and price_45m > ema21_45m * 0.998
                    c1h = current_1h.get('ema_9', 0) > ema21_1h and price_1h > ema21_1h * 0.998
                    c4h = current_4h.get('ema_9', 0) > ema21_4h and price_4h > ema21_4h * 0.998
                    logger.debug(f"✅ MTF {symbol} LONG check 45m={c45} 1h={c1h} 4h={c4h}")
                    if not (c45 and c1h and c4h):
                        logger.warning(f"🚫 {symbol}: MTF проверка НЕ ПРОЙДЕНА для LONG: 45m={c45} 1h={c1h} 4h={c4h}")
                    return c45 and c1h and c4h
                if dir_ == 'sell':
                    # УЖЕСТОЧЕНО: Проверяем EMA + цена ниже EMA21 на всех таймфреймах
                    price_45m = current_45m.get('price', 0)
                    price_1h = current_1h.get('price', 0)
                    price_4h = current_4h.get('price', 0)
                    ema21_45m = current_45m.get('ema_21', 0)
                    ...
                    c45 = current_45m.get('ema_9', 0) < ema21_45m and price_45m < ema21_45m * 1.002
                    c1h = current_1h.get('ema_9', 0) < ema21_1h and price_1h < ema21_1h * 1.002
                    c4h = current_4h.get('ema_9', 0) < ema21_4h and price_4h < ema21_4h * 1.002
                    logger.debug(f"✅ MTF {symbol} SHORT check 45m={c45} 1h={c1h} 4h={c4h}")
                    if not (c45 and c1h and c4h):
                        logger.warning(f"🚫 {symbol}: MTF проверка НЕ ПРОЙДЕНА для SHORT: 45m={c45} 1h={c1h} 4h={c4h}")
                    return c45 and c1h and c4h
                return False

            if signal and not _mtf_confirm(signal):
                logger.warning(f"🚫 {symbol}: ОТКЛОНЕНО — нет подтверждения 45m+1h+4h для {signal.upper()} | БЛОКИРОВКА ВХОДА")
                signal = None
                confidence = 0  # Сбрасываем уверенность, чтобы сигнал точно не прошел

            # ДОП. ФИЛЬТРЫ ДЛЯ НОВЫХ СДЕЛОК: требуем реальный потенциал для +1% (УЖЕСТОЧЕНО)
            def _has_min_potential(direction: str) -> bool:
                try:
                    ...
                    # УЖЕСТОЧЕНО: Требуемая волатильность > 2.0% (было 1.2%)
                    if atr_pct < 2.0:
                        logger.info(f"🚫 {symbol}: Отклонено | ATR45m={atr_pct:.2f}% < 2.0% (требуется минимум 2.0%)")
                        return False
                    # УЖЕСТОЧЕНО: Объем > 1.5x (было 1.2x)
                    if vol_ratio < 1.5:
                        logger.info(f"🚫 {symbol}: Отклонено | VolumeRatio45m={vol_ratio:.2f} < 1.5 (требуется минимум 1.5x)")
                        return False
                    # УЖЕСТОЧЕНО: Направление импульса на 15m/30m + цена относительно EMA21
                    ...
                    price_15m = current_15m.get('price', 0)
                    price_30m = current_30m.get('price', 0)
                    ema21_15m = current_15m.get('ema_21', 0)
                    ema21_30m = current_30m.get('ema_21', 0)
                    
                    if direction == 'buy':
                        if not (ema_up_15 and ema_up_30):
                            logger.info(f"🚫 {symbol}: Отклонено | Нет импульса на 15m/30m для LONG")
                            return False
                        # Цена должна быть выше EMA21 на обоих таймфреймах
                        if not (price_15m > ema21_15m * 0.997 and price_30m > ema21_30m * 0.997):
                            logger.info(f"🚫 {symbol}: Отклонено | Цена ниже EMA21 на 15m/30m для LONG")
                            return False
                    if direction == 'sell':
                        if not (ema_dn_15 and ema_dn_30):
                            logger.info(f"🚫 {symbol}: Отклонено | Нет импульса на 15m/30m для SHORT")
                            return False
                        # Цена должна быть ниже EMA21 на обоих таймфреймах
                        if not (price_15m < ema21_15m * 1.003 and price_30m < ema21_30m * 1.003):
                            logger.info(f"🚫 {symbol}: Отклонено | Цена выше EMA21 на 15m/30m для SHORT")
                            return False
            ...
            # КРИТИЧЕСКАЯ ПРОВЕРКА: Даже если все условия выполнены, повторно проверяем MTF
            # Это гарантирует, что НИ ОДИН сигнал не пройдет без 45m+1h+4h подтверждения
            if signal and confidence >= adaptive_min_confidence and _has_min_potential(signal):
                # ФИНАЛЬНАЯ ПРОВЕРКА MTF - НИКАКИЕ ИСКЛЮЧЕНИЯ
                if not _mtf_confirm(signal):
                    logger.error(f"🚨 КРИТИЧЕСКАЯ ОШИБКА: {symbol} - сигнал {signal.upper()} прошёл все фильтры, но MTF НЕ ПОДТВЕРЖДЁН! БЛОКИРОВКА!")
                    logger.error(f"   Это не должно было произойти. Проверьте логику входа!")
                    return None
                
                # ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: Убеждаемся, что все таймфреймы согласованы
                # Это предотвращает входы при нестабильном рынке
                if signal == 'buy':
                    # Для LONG проверяем, что все таймфреймы действительно бычьи
                    mtf_alignment = (
                        current_15m.get('ema_9', 0) > current_15m.get('ema_21', 0) and
                        current_30m.get('ema_9', 0) > current_30m.get('ema_21', 0) and
                        current_45m.get('ema_9', 0) > current_45m.get('ema_21', 0) and
                        current_1h.get('ema_9', 0) > current_1h.get('ema_21', 0) and
                        current_4h.get('ema_9', 0) > current_4h.get('ema_21', 0)
                    )
                else:
                    # Для SHORT проверяем, что все таймфреймы действительно медвежьи
                    mtf_alignment = (
                        current_15m.get('ema_9', 0) < current_15m.get('ema_21', 0) and
                        current_30m.get('ema_9', 0) < current_30m.get('ema_21', 0) and
                        current_45m.get('ema_9', 0) < current_45m.get('ema_21', 0) and
                ...
                if not mtf_alignment:
                    logger.warning(f"🚫 {symbol}: Отклонено | Неполное MTF согласование всех таймфреймов (15m/30m/45m/1h/4h)")
                    logger.warning(f"   Это предотвращает входы при нестабильном рынке (как AI16ZUSDT)")
                    return None
...
🎯 ТП: старт +1% → трейлинг 0.5% → +4% (\$1 гарантированно) → до +5%"""
    ...
    async def open_position_automatically(self, signal: EnhancedSignal) -> bool:
        ...
        try:
            ...
            def normalize_symbol_check(sym: str) -> str:
                if not sym:
                    return sym
                norm = sym.upper().replace('/', '').replace('-', '').replace(':', '')
                if norm.endswith('USDTUSDT'):
                    norm = norm[:-4]  # Убираем дублирование USDT
                return norm
            
            symbol_norm = normalize_symbol_check(symbol)
            
            # Проверяем в active_positions с нормализацией
            for active_symbol, pos_info in list(self.active_positions.items()):
                active_symbol_norm = normalize_symbol_check(active_symbol)
                if active_symbol_norm == symbol_norm:
                    logger.error(f"🚨 КРИТИЧЕСКАЯ ОШИБКА: {symbol} уже в active_positions как {active_symbol}")
                    return False
            
            # КРИТИЧЕСКАЯ ПРОВЕРКА: Получаем ВСЕ позиции с биржи и проверяем с нормализацией
            try:
                all_positions = await self.exchange.fetch_positions(params={'category': 'linear'})
                for pos in all_positions:
                    pos_symbol = pos.get('symbol', '')
                    pos_symbol_norm = normalize_symbol_check(pos_symbol)
            ...
            try:
                ...
                # V4.0: TP контролируется универсальным монитором
                # Монитор устанавливает: старт +1% ($0.25) → трейлинг 0.5% → +4% ($1.00 гарантированно) → +5% ($1.25)
                # Гарантированный минимум +$1 достигается при +4% на позицию $25
                tp_percent = 1.15  # Стартовый TP +1.15% (монитор трейлит до +4% для гарантии +$1, затем до +5%)
                if signal.direction == 'buy':
                    tp_price = entry_price * (1 + tp_percent / 100.0)
                else:
                    tp_price = entry_price * (1 - tp_percent / 100.0)
                tp_prices = [tp_price]  # Стартовый TP +1%, монитор будет обновлять
                ...
                if not sl_tp_set:
                    logger.warning(f"⚠️ {symbol}: SL/TP не установлены на бирже. Универсальный монитор установит их в течение минуты.")
                else:
                    logger.info(f"✅ {symbol}: SL/TP установлены на бирже (стартовые: TP +1%, SL -$1). Монитор будет обновлять TP с трейлингом.")
    ...
    async def monitor_positions(self):
        ...
        try:
            ...
            # Удаляем закрытые позиции из словаря
            # НОРМАЛИЗАЦИЯ СИМВОЛОВ: приводим все к единому формату для корректного сравнения
            def normalize_symbol_cooldown_check(sym: str) -> str:
                """Нормализует символ для cooldown списка"""
                if not sym:
                    return sym
                norm = sym.upper().replace('/', '').replace('-', '').replace(':', '')
                if norm.endswith('USDTUSDT'):
            ...
            def normalize_symbol(sym: str) -> str:
                """Нормализует символ для сравнения (ICNTUSDT = ICNT/USDT:USDT)"""
                if not sym:
                    return sym
                norm = sym.upper().replace('/', '').replace('-', '')
                if norm.endswith(':USDT'):
            ...
            active_symbols = {normalize_symbol(p.get('symbol', '')) for p in open_positions if (p.get('contracts', 0) or p.get('size', 0)) > 0}
            ...
            prev_open_normalized = {normalize_symbol(s) for s in self._prev_open_positions}
            active_positions_normalized = {normalize_symbol(s) for s in self.active_positions.keys()}
                    ...
                    if not pos_info:
                        ...
                        try:
                            ...
                            if items:
                                ...
                                avg_entry = float(it.get('avgEntryPrice') or 0)
                                avg_exit = float(it.get('avgExitPrice') or 0)
                                ex_side = it.get('side', 'Buy')
                                ...
                                if profit_usd < -0.5:  # Если убыток больше $0.50
                                    symbol_norm_loss = normalize_symbol_cooldown_check(symbol)
    ...
    async def trading_loop_v4(self):
        ...
        try:
            ...
            for symbol in symbols:
                ...
                try:
                    # Нормализуем символ для проверки
                    def normalize_symbol_check(sym: str) -> str:
                        if not sym:
                            return sym
                        norm = sym.upper().replace('/', '').replace('-', '').replace(':', '')
                ...
                def normalize_symbol_cooldown(sym: str) -> str:
                    if not sym:
                        return sym
                    norm = sym.upper().replace('/', '').replace('-', '').replace(':', '')
                    if norm.endswith('USDTUSDT'):
                ...
                symbol_norm_cooldown = normalize_symbol_cooldown(symbol)
...
🎯 *TP: старт +1% → трейлинг 0.5% → +4% (\$1 гарантированно) → до +5%*
