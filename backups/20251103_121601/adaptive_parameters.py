#!/usr/bin/env python3
    ...
    def get_trading_parameters(self) -> Dict:
        ...
        # Адаптируем параметры к текущим рыночным условиям
        if self.current_market_condition:
            if self.current_market_condition.volatility > 0.7:
                # При высокой волатильности уменьшаем размер позиции
                position_size = self.trading_params.position_size * 0.8
                take_profit_percent = self.trading_params.take_profit_percent * 1.2
                trailing_percent = self.trading_params.trailing_percent * 1.2
            elif self.current_market_condition.trend == 'bullish':
                # В бычьем тренде увеличиваем размер позиции
                position_size = self.trading_params.position_size * 1.2
                take_profit_percent = self.trading_params.take_profit_percent * 1.1
                trailing_percent = self.trading_params.trailing_percent
            elif self.current_market_condition.trend == 'bearish':
                # В медвежьем тренде уменьшаем риски
                position_size = self.trading_params.position_size * 0.8
                take_profit_percent = self.trading_params.take_profit_percent * 0.9
                trailing_percent = self.trading_params.trailing_percent * 0.9
            else:
                # Стандартные параметры
                position_size = self.trading_params.position_size
                take_profit_percent = self.trading_params.take_profit_percent
                trailing_percent = self.trading_params.trailing_percent
        else:
            # Если нет данных о состоянии рынка, используем стандартные параметры
            position_size = self.trading_params.position_size
            take_profit_percent = self.trading_params.take_profit_percent
            trailing_percent = self.trading_params.trailing_percent
        ...
        return {
            ...
            'stop_loss_amount': self.trading_params.stop_loss_amount,
...
class TradingParameters:
    """💰 Параметры торговли"""
    position_size: float = 30.0  # Размер позиции в USD
    take_profit_percent: float = 2.0  # Тейк-профит в процентах
    trailing_percent: float = 1.0  # Процент трейлинга
    stop_loss_amount: float = 1.0  # Стоп-лосс в USD
    leverage: int = 10  # Кредитное плечо 10x
...
class AdaptiveParameterSystem:
    ...
    def _adapt_to_volume(self, params: AdaptiveParameters, volume_trend: str) -> AdaptiveParameters:
        """📊 Адаптация к тренду объёма"""
        if volume_trend == 'high':
            # Высокий объём = можно быть менее строгими
            volume_factor = 0.9
        elif volume_trend == 'low':
