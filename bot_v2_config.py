...
class Config:
    ...
    # ========================================
    TEST_MODE = True                    # 🧪 ТЕСТОВЫЙ РЕЖИМ ДЛЯ ПРОВЕРКИ
    ...
    COOLDOWN_AFTER_TRADE_SECONDS = 86400  # 24 ЧАСА после сделки (КРИТИЧНО!)
    MAX_TRADES_PER_DAY = 3            # Максимум 3 сделки/день (качество > количество!)
    
    # ========================================
    # 🔍 ФИЛЬТРЫ СИГНАЛОВ (СБАЛАНСИРОВАННЫЕ!)
    # ========================================
    MIN_CONFIDENCE_PERCENT = 92        # 92% минимум (КАЧЕСТВЕННЫЕ СИГНАЛЫ!)
    MIN_SIGNAL_STRENGTH = 0.85         # 85% сила сигнала (РАЗУМНО!)
    
    # ДОПОЛНИТЕЛЬНЫЕ ФИЛЬТРЫ ДЛЯ КАЧЕСТВА:
    MIN_VOLUME_24H = 10000000          # Минимум $10M объём за 24ч (УДВОЕНО!)
    MIN_PRICE = 0.01                   # Минимум $0.01 за монету
    MAX_SPREAD_PERCENT = 0.5           # Максимум 0.5% спред
    
    # ТРЕБОВАНИЯ К СИГНАЛУ:
    MIN_INDICATORS_AGREE = 4           # Минимум 4 индикатора (РАЗУМНО!)
    REQUIRE_TREND_CONFIRMATION = True  # Требовать подтверждение тренда
    REQUIRE_VOLUME_CONFIRMATION = True # Требовать подтверждение объёмом
