...
class TradingBotV2:
    ...
    async def trading_loop(self):
        ...
        try:
            ...
            # 7. Получаем ВСЕ доступные монеты с фильтрацией по ликвидности
            logger.info("📊 Получаем все доступные монеты с фильтрацией по ликвидности...")
            ...
            await self.send_telegram(
                f"🔄 *ТОРГОВЫЙ ЦИКЛ*\n\n"
                f"⏰ {current_time}\n"
                f"📊 Все доступные монеты (исключая неликвидные)\n"
                f"🔍 Начинаю анализ...\n"
                f"⏱️ ~5 минут"
            )
            
            # Получаем все доступные монеты с анализом волатильности
            if enhanced_symbol_selector.is_cache_valid():
                ...
                # Получаем свежий анализ всех доступных монет
                volatile_symbols_data = await enhanced_symbol_selector.get_volatile_symbols(
                    exchange_manager, top_n=200  # Увеличиваем до 200 для большего охвата
                )
            ...
            await self.send_telegram(
                ...
                f"⏰ {current_time}\n"
                f"📊 Проанализировано: {len(symbols)} монет (все доступные)\n"
                f"💎 Сильных сигналов (≥{Config.MIN_CONFIDENCE_PERCENT}%): {strong_signals_found}\n"
                f"🚀 Охват: максимальный (исключены только неликвидные)"
            )
            ...
            if not best_signal:
                ...
                f"⏰ {current_time}\n"
                f"📊 Проверено: {len(symbols)} монет (все доступные)\n"
                f"🎯 Сигналов ≥{Config.MIN_CONFIDENCE_PERCENT}%: {strong_signals_found}\n"
                f"🧠 AI выбрал: 0\n"
                f"🚀 Охват: максимальный\n\n"
